...
export default function Waiters() {
  const { waiters, loadWaiters } = useApp()
  ...
  const handleSave = async () => {
    ...
    let result
    if (editItem) {
      result = await window.api.waiters.update(editItem.id, form)
    } else {
      result = await window.api.waiters.create(form)
    }
    setLoading(false)
    if (result.success) { setShowModal(false); loadWaiters() }
    else setError(result.message || 'Failed to save')
  }

  const handleDelete = async (id) => {
    if (!confirm('Delete this waiter?')) return
    const result = await window.api.waiters.delete(id)
  ...
  return (
    <div className="p-4 h-full overflow-y-auto">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-gray-800">Waiters</h2>
        <Button onClick={openCreate} size="sm">+ Add Waiter</Button>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <DataTable columns={columns} data={waiters} emptyMessage="No waiters found" />
      </div>
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Waiter' : 'New Waiter'} size="sm">
